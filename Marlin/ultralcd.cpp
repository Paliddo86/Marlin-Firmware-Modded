...
void kill_screen(const char* lcd_msg) {
   ...
   *
   */
static void lcd_prepare_glass_menu(){
      enqueue_and_echo_commands_P(PSTR("G0 F9000 X98 Y175 Z60"));
      lcd_return_to_status();
      }
  ...
  void lcd_prepare_menu() {
    ...
    //
    //Change Filament Manual(Paliddos)
    //
    
    MENU_ITEM(gcode, MSG_CHANGE_FIL, PSTR("M600 X98 Y85 Z50 E-2 L-100"));

    //Preparazione vetro
    MENU_ITEM(function, MSG_GLASS, lcd_prepare_glass_menu);
    #if HAS_M206_COMMAND
      //
      // Set Home Offsets
      //
   ...
   */
    ...
    void lcd_sdcard_menu() {
      ...
      //Test reset filament (Paliddos)
      static void quattrocento(){
          double spool = 400000;
        print_job_timer.setSpool(spool);  
        }
      static void sessanta(){
          double spool = 60000;
        print_job_timer.setSpool(spool);  
        }
        static void trenta(){
      ...
      static void lcd_filament_count_reset(){
        print_job_timer.resetFilament();
        }
        int setamount=0;
      static void filament_amount(){
        double fil_conv=(double) setamount;
            double filament_set=fil_conv*1000;
            print_job_timer.setFilament(filament_set);
            }
        int usata=0;
        static void set_used_spool(){
          double usedspool = (double) usata;
          double spool=usedspool*1000;
          print_job_timer.setSpool(spool);
          }    
       static void set_spool(){
        START_MENU();
        MENU_BACK(MSG_MAIN);
        MENU_ITEM(function, "Bobina 1Kg", quattrocento);
        MENU_ITEM(function, "Bobina 60m", sessanta);
        MENU_ITEM(function, "Bobina 30m", trenta);
        MENU_ITEM_EDIT(int3, "Bobina Usata", &usata,0,200);
        MENU_ITEM(function, "Imposta Bobina", set_used_spool);
        END_MENU();
      }
      static void lcd_info_filament(){
        START_MENU();
        MENU_BACK(MSG_MAIN);
        MENU_ITEM(function, MSG_FILAMENT_COUNT_RESET, lcd_filament_count_reset);
        MENU_ITEM_EDIT(int3 , MSG_ADD_FILAMENT, &setamount, 0, 400);
        MENU_ITEM(function, MSG_SET_FILAMENT,filament_amount);
        MENU_ITEM(submenu, MSG_LOAD_SPOOL,set_spool);
        END_MENU();
    ...
    void lcd_info_menu() {
      ...
      MENU_ITEM(submenu, MSG_INFO_THERMISTOR_MENU, lcd_info_thermistors_menu); // Thermistors >
        ...
        MENU_ITEM(submenu, MSG_INFO_FILAMENT_MENU , lcd_info_filament);         // Filament setting count(Paliddo)
      #endif
      END_MENU();
    }
  #endif // LCD_INFO_MENU

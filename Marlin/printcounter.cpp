...
void PrintCounter::incFilamentUsed(double const &amount) {
  ...
  current_filament_used += amount;
}
//Add filament used in spool(paliddo)
void PrintCounter::setFilament(double const &setamount) {
  #if ENABLED(DEBUG_PRINTCOUNTER)
    PrintCounter::debug(PSTR("setFilament"));
  #endif

  this->loaded = true;
  this->data.filamentUsed=setamount; //mm
  this->saveStats();
  eeprom_write_byte((uint8_t *) this->address, 0x16);
}
//Add spool lenght
void PrintCounter::setSpool(double const &spool) {
  #if ENABLED(DEBUG_PRINTCOUNTER)
    PrintCounter::debug(PSTR("setSpool"));
  #endif

  this->loaded = true;
  this->data.spoolTotal=spool; //mm
  this->saveStats();
  eeprom_write_byte((uint8_t *) this->address, 0x16);
}

//Reset Filament for new spool
void PrintCounter::resetFilament() {
  #if ENABLED(DEBUG_PRINTCOUNTER)
    PrintCounter::debug(PSTR("resetFilament"));
  #endif

  this->loaded = true;
  this->data.filamentUsed=0;
}

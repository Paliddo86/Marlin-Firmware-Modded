...
static void lcd_implementation_status_screen() {
  ...
  if (PAGE_UNDER(28)) {
    ...
    #endif
// Filament Info(Paliddo)

//Filament Remain in spool

printStatistics stats = print_job_timer.getStats();
  u8g.setPrintPos(29,8);
  u8g.print("RS: ");
  float rs = (stats.spoolTotal - stats.filamentUsed)/1000;
  if(rs>99){
u8g.setPrintPos(50,8);
  u8g.print(rs,0);
  u8g.print("m");
  }else{
  u8g.setPrintPos(50,8);
...
//Filament needed  
  ...
  u8g.print("TS: ");
  float ts = stats.spoolTotal/1000;
  if(ts>99){
